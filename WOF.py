...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        karguess = self.getPossibleLetters(guessed)
        if len(karguess) == 0:
            return 'pass'
...
def getNumberBetween(prompt, minimum, maximum):
    ...
    while True:
        try:
            ...
            if n < minimum:
                errmessage = 'Must be at least {}'.format(minimum)
            elif n > maximum:
                errmessage = 'Must be at most {}'.format(maximum)
            else:
                return n
...
def getRandomCategoryAndPhrase():
    with open("phrases.txt", 'r') as f:
        cat = json.dumps(f.read())
        phrases = json.loads(cat)
...
num_human = getNumberBetween('Enter the number of human players: ', 1, 10)
...
if num_computer >= 1:
    difficulty = getNumberBetween('Choose difficulty (1-10) : ', 1, 10)
...
if winner:
    ...
    if len(winner.prizes) > 0:
        ...
        for prize in set(winner.prizes):
            print('    - {}'.format(prize))
else:
